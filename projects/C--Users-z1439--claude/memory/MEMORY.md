...
- [beh-pref-001](root\beh_pref_negative_constraints.md) — 行為規則必須以否定約束格式撰寫（不准 X），正面指令對編碼任務有害（−14.3pp） [1.00 fresh]
- [beh-pref-three-files](root\beh_pref_three_files.md) — 所有行為配置只通過三個文件 — settings.json, CLAUDE.md, AGENTS.md [1.00 fresh]
- [mem-pref-001](root\mem_pref_decay_scoring.md) — 記憶新鮮度使用 Ebbinghaus 遺忘曲線評分，基於創建時間、訪問次數、最近訪問時間 [1.00 fresh]
- [mem-pref-002](root\mem_pref_max_timestamp.md) — 記憶衝突解決使用 max(timestamp) 確定性方法（94.8%），不準用 LLM 判斷（54%） [1.00 fresh]

## Branch (Active Topics)
> 活躍主題。上限 20 條。

- [beh-pref-003](branch\beh_pref_save_experience.md) — 每一次会话结束前必须主动攒经验写入 memory，不准只干活不记录 [1.00 fresh]
- [beh-pref-004](branch\pref_tool_batch_config.md) — 工具/MCP 配置类任务要并行调研、一次性全部配完，不准逐个询问用户 [1.00 fresh]
...
- [fb-session-lessons](leaf\fb_session_lessons.md) — ~~2026-06-24 session 核心教訓~~ → superseded by [1.00 fresh]
- [fb-thinking-process](leaf\fb_thinking_process.md) — ~~動手前思考流程~~ → superseded by [1.00 fresh]
- [fb-sys-proj](leaf\feedback_system_vs_project.md) — 混淆了自身系统改进和用户项目改进，导致范围扩展和自欺欺人 [1.00 fresh]
- [fb-scope-001](leaf\feedback_task_scope.md) — ~~用户让做什么就做什么~~ → superseded by [1.00 fresh]
- [ref-mcp-001](leaf\ref_mcp_config_locations.md) — MCP Server 配置的三层位置（settings.json / .mcp.json / plugins marketplace）及当前已配工具清单 [1.00 fresh]
